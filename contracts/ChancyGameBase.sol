...
abstract contract ChancyGameBase is IEntropyConsumer {
    ...
    constructor(address gameTokenAddress, address entropyAddress) {
        require(gameTokenAddress != address(0), "INVALID_TOKEN");
    ...
    function fundSessionRewards(uint256 sessionId, uint256 amount) external {
        ...
        require(gameToken.balanceOf(address(this)) >= amount, "REWARD_RESERVE_NOT_RECEIVED");
    ...
    function joinSession(uint256 sessionId, bytes32 userRandomNumber) external payable {
        ...
        gameToken.safeTransferFrom(msg.sender, address(this), session.entryAmount);
